...
-- TODO
-- optimze: we should not touch overlapping cells, this will reduce GC work on magnitude scale
local function queryUpdate( node, level, x, y, w, h, rectid, nrectid )
	return queryModify( queryModify( node, level, x, y, w, h, rectid, remove ), x, y, w, h, nrectid, insert )
end

function QuadTree.insert( self, x, y, w, h )
	local o = copy( self )
	o.id = o.id+1
	local rectid = {x, y, x+w, y+h, o.id}
...
function QuadTree.update( self, rectid, nx, ny, nw, nh )
	...
	local o, nrectid = copy( self ), {nx, ny, nx+nw, ny+nh, rectid[5]}
	o.root = queryUpdate( self, self.levels, 0, 0, self.width, self.height, rectid, nrectid )
	return o, nrectid
end
